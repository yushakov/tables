...
const App = () => {
  ...
  useEffect(() => {
    if ('viewMode' in localStorage) {
      let mode = localStorage.getItem('viewMode') as ViewMode | ViewMode.Day;
      setView(mode);
    }
    else {
      localStorage.setItem('viewMode', ViewMode.Day);
  ...
  const handleViewSwitch = (viewMode: React.SetStateAction<ViewMode>) => {
    setView(viewMode);
    localStorage.setItem('viewMode', String(viewMode));
  };

  return (
    <div className="Wrapper">
      <ViewSwitcher
        onViewModeChange={(viewMode) => {handleViewSwitch(viewMode)}}
        onViewListChange={setIsChecked}
        isChecked={isChecked}
      />
      {window.interactive && <div dangerouslySetInnerHTML={{ __html: displayText }} />}
      <Gantt
        tasks={tasks}
        viewMode={view}
      ...
      />
      {window.interactive && <button onClick={handleSubmit}>Submit Changes</button>}
    </div>
  );
};
