...
declare global {
  interface Window {
    construct_id: number; // Specify the type of your project_id
  }
}
...
const App = () => {
  ...
  useEffect(() => {
    axios.get<Choice[]>(`http://127.0.0.1:8000/gantt/api/choices/?id=${construct_id}`)
      .then(response => {
        const transformedTasks: Task[] = response.data.map((choice: Choice) => {
          ...
          return {
            ...
            name: choice.name_txt.slice(0, 20),
            start: new Date(choice.plan_start_date),
            end: endDate,
            progress: choice.progress_percent_num,
            type: "task",  // Assuming all choices are of type "task"
        ...
        const tsk: Task = {
          ...
          name: transformedTasks[0].project!,
          id: transformedTasks[0].project!,
          progress: 0,
          type: "project",
          hideChildren: false,
          displayOrder: 1,
        };
        const [start, end] = getStartEndDateForProject(transformedTasks, tsk.id);
        tsk.start = start;
        tsk.end = end;
        transformedTasks.push(tsk);
        //transformedTasks[0] = tsk;
